...
class DigitalTyphoonSequence:
    ...
    def process_seq_img_dir_into_sequence(
        self,
        directory_path: str,
        ...
        for root, dirs, files in os.walk(directory_path, topdown=True):
            ...
            for filepath, file_sequence, file_date, file_satellite in filepaths:
                if filepath not in ignore_list:
                    self.datetime_to_image[file_date].set_image_data(
                        self.img_root / filepath,
        ...
        if self.verbose:
            if not self.num_images_match_num_expected():
                warnings.warn(
                    f"The number of images ({len(self.images)}) does not match the "
