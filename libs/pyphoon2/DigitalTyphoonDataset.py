...
from typing import List, Sequence, Union, Optional, Dict, Callable, Tuple
...
from libs.pyphoon2.DigitalTyphoonUtils import (
    _verbose_print,
    SPLIT_UNIT,
    LOAD_DATA,
    TRACK_COLS,
    get_seq_str_from_track_filename,
)
...
class DigitalTyphoonDataset(Dataset):
    ...
    def __init__(
        ...
        sequenceFilterWhenPopImageToSequences: Callable[[str], bool] | None = None,
        transform_func=None,
        transform=None,
        verbose=False,
    ) -> None:
        ...
        if not SPLIT_UNIT.has_value(split_dataset_by):
            raise ValueError(
                f"Split unit must one of the following\n"
        ...
        if not LOAD_DATA.has_value(load_data_into_memory):
            raise ValueError(
                f"Load data option must one of the following\n"
        ...
        self.sequenceFilterWhenPopImageToSequences = (
            sequenceFilterWhenPopImageToSequences
        )
        ...
        self.number_of_images = (
            0  # number of images in the dataset, after augmentation and removal
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.get_images_by_sequence:
            ...
            labels = np.array(
                [self._labels_from_label_strs(image, self.labels) for image in images]
            )
            if self.transform:
    ...
    def random_split(
        ...
        _verbose_print(
            f"Splitting the dataset into proportions {lengths}, by {split_by}.",
            verbose=self.verbose,
        )

        if not SPLIT_UNIT.has_value(split_by):
            warnings.warn(
                f"Split unit '{split_by}' is not within the list of known split units: "
    ...
    def labels_as_tensor(self, indices: List[int], label: str) -> torch.Tensor:
        ...
        """
        images = [
            self.get_image_from_idx(idx).value_from_string(label) for idx in indices
        ]
        return torch.Tensor(images)
    ...
    def seq_indices_to_total_indices(
        self, seq_obj: DigitalTyphoonSequence
    ) -> List[int]:
        """
        ...
        return [
            i + self._seq_str_to_first_total_idx[seq_str]
            for i in range(seq_obj.get_num_images())
        ]
    ...
    def _sequenceFilterWhenPopImageToSequences(self, sequenceID: str) -> None:
        if self.sequenceFilterWhenPopImageToSequences:
            if self.sequenceFilterWhenPopImageToSequences(sequenceID):
                return True
            return False
        return True
    ...
    def _populate_images_into_sequences(self, image_dir: str) -> None:
        ...
        for root, dirs, files in os.walk(image_dir, topdown=True):
            for dir_name in sorted(
                ...
                if not self._sequenceFilterWhenPopImageToSequences(dir_name):
                    continue
        ...
        for sequence in self.sequences:
            ...
            if not sequence.num_images_match_num_expected():
                if self.verbose:
                    warnings.warn(
                        f"Sequence {sequence.sequence_str} has only {sequence.get_num_images()} when "
        ...
        for root, dirs, files in os.walk(metadata_dir, topdown=True):
            for file in sorted(files):
                ...
                if self.sequence_exists(file_sequence):
                    self._get_seq_from_seq_str(file_sequence).set_track_path(
                        root + file
                    )
    ...
    def _read_in_track_file_to_sequence(
        self, seq_str: str, file: str, csv_delimiter=","
    ) -> DigitalTyphoonSequence:
        """
    ...
    def _random_split_by_season(
        ...
        non_empty_season_indices = [
            non_empty_season_indices[idx]
            for idx in randperm(len(non_empty_season_indices), generator=generator)
        ]
        randomized_season_list = [
            list(self.season_to_sequence_nums.keys())[i]
            for i in non_empty_season_indices
        ]
        ...
        while season_iter < len(randomized_season_list):
            if (
                len(return_indices_sorted[bucket_counter][2])
                < return_indices_sorted[bucket_counter][0]
            ):
                for seq in self.season_to_sequence_nums[
                    randomized_season_list[season_iter]
                ]:
                    sequence_obj = self._get_seq_from_seq_str(seq)
                    if self.get_images_by_sequence:
                        if (
                            sequence_obj.get_num_images() > 0
                        ):  # Only append if the sequence has images
                        ...
                        return_indices_sorted[bucket_counter][2].extend(
                            self.seq_indices_to_total_indices(
                                self._get_seq_from_seq_str(seq)
                            )
                        )
                season_iter += 1
            bucket_counter += 1
        ...
        return [
            Subset(self, bucket_indices)
            for _, _, bucket_indices in return_indices_sorted
        ]

    def _random_split_by_sequence(
        self,
        lengths: Sequence[Union[int, float]],
        generator: Optional[Generator] = default_generator,
    ) -> List[Subset]:
        ...
        non_empty_sequence_indices = [
            ...
            if self.sequences[idx].get_num_images() > 0
        ]
        randomized_seq_indices = [
            non_empty_sequence_indices[idx]
            for idx in randperm(len(non_empty_sequence_indices), generator=generator)
        ]

        bucket_counter = 0
        seq_iter = 0
        ri = len(randomized_seq_indices)
        while seq_iter < ri:
            # print("len(randomized_seq_indices)", ri, "seq_iter", seq_iter)
            if (
                len(return_indices_sorted[bucket_counter][2])
                < return_indices_sorted[bucket_counter][0]
            ):
    ...
    def _get_image_from_idx_as_numpy(self, idx) -> np.ndarray:
        ...
        sequence_str = self._find_sequence_str_from_image_index(idx)
        sequence = self._get_seq_from_seq_str(sequence_str)
        return sequence.get_image_at_idx_as_numpy(
            self.total_image_idx_to_sequence_idx(idx)
        )
    ...
    def _labels_from_label_strs(self, image: DigitalTyphoonImage, label_strs):
        ...
        if (type(label_strs) is list) or (type(label_strs) is tuple):
            label_ray = np.array(
                [image.value_from_string(label) for label in label_strs]
            )
            return label_ray
        else:
